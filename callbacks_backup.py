...
Вы уверены, что хотите продолжить?
    ...
    async def schedule_next_day_opening(self, user_id: int, next_day: int, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.info(f"Запланировано открытие Дня {next_day} для пользователя {user_id} на {tomorrow_6am}")
            
        except Exception as e:
            logger.error(f"Ошибка планирования открытия следующего дня: {e}")
