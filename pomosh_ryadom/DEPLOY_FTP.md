...
```bash
cd /var/www/pomosh_ryadom
git clone https://github.com/andreyburylov59/DianaLisaBot.git .
git checkout cursor/stage-5-geolocation-4496
```
...
```bash
cd /var/www/pomosh_ryadom
git pull origin cursor/stage-5-geolocation-4496
source venv/bin/activate
pip install -r requirements_production.txt
sudo systemctl restart pomosh_ryadom
```
